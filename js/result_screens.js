...
function ensureScreens(){const main=document.querySelector('main.container'),result=$('result');if(!main||!result)return null;let rs=$('mapoResultScreen');if(!rs){rs=document.createElement('section');rs.id='mapoResultScreen';rs.className='result-screen';rs.hidden=true;main.appendChild(rs);}let ts=$('reportTablesScreen');if(!ts){ts=document.createElement('section');ts.id='reportTablesScreen';ts.className='result-screen';ts.hidden=true;main.appendChild(ts);}if(result.parentElement!==rs)rs.appendChild(result);const panel=$('reportTablesPanel');if(panel&&panel.parentElement!==ts)ts.appendChild(panel);return{rs,ts};}
