...
function bindReportActions(){
 document.addEventListener('click',function(ev){
   const target=ev.target.closest?.('#openReportTables, #generateReportTables');
   if(!target)return;
   if(target.id==='openReportTables'){
     ev.preventDefault();
     showTables();
     return;
   }
   if(target.id==='generateReportTables'){
     ev.preventDefault();
     if(typeof window.MAPOReportTables?.render==='function')window.MAPOReportTables.render();
   }
 });
}
...
function init(){ensureScreens();addNav();bindReportActions();watch();}
