...
function showResult(){return open('result',el=>{if(typeof window.MAPOResultsUI?.render==='function')window.MAPOResultsUI.render();});}
...
function bindLegacy(){document.addEventListener('click',e=>{const legacy=e.target.closest('#openMapoSimulation,#openReportTables,#createReportTables');if(!legacy)return;e.preventDefault();e.stopImmediatePropagation();legacy.id==='openMapoSimulation'?showSimulation():showTables();},true);}
