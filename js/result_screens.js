/* MAPO Analyzer — pantallas independientes de resultado y tablas.
   Solo controla presentación/navegación. No modifica ningún cálculo. */
(function(){'use strict';
 ...
 function showResult(){const s=ensureScreens();if(!s)return;hideWork();s.ts.hidden=true;s.rs.hidden=false;const r=$('result');r.hidden=false;window.MAPOResultsUI?.render?.();}
 ...
 function addNav(){const s=ensureScreens();if(!s)return;
   if(!document.getElementById('resultScreenActions')){const a=document.createElement('div');a.id='resultScreenActions';a.className='actions result-screen-actions';a.innerHTML='<button type="button" id="backToStudyFromResult" class="secondary">Volver a la evaluación</button><button type="button" id="goToReportTables">Ver tablas para informe Word</button>';s.rs.appendChild(a);a.querySelector('#backToStudyFromResult').onclick=showStudy;a.querySelector('#goToReportTables').onclick=showTables;}
 ...
 function watch(){const r=$('result');if(!r)return;new MutationObserver(()=>{if(!r.hidden)showResult();}).observe(r,{attributes:true,attributeFilter:['hidden']});}
 function init(){ensureScreens();addNav();watch();}
