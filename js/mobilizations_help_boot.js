...
(function(){
 ...
 const screens=[
  ['manualTotal',0,'Movilizaciones manuales con levantamiento total — Mañana'],['manualTotal',1,'Movilizaciones manuales con levantamiento total — Tarde'],['manualTotal',2,'Movilizaciones manuales con levantamiento total — Noche'],
  ['manualPartial',0,'Movilizaciones manuales con levantamiento parcial — Mañana'],['manualPartial',1,'Movilizaciones manuales con levantamiento parcial — Tarde'],['manualPartial',2,'Movilizaciones manuales con levantamiento parcial — Noche'],
  ['aidedTotal',0,'Movilizaciones con ayuda — levantamiento total — Mañana'],['aidedTotal',1,'Movilizaciones con ayuda — levantamiento total — Tarde'],['aidedTotal',2,'Movilizaciones con ayuda — levantamiento total — Noche'],
  ['aidedPartial',0,'Movilizaciones con ayuda — levantamiento parcial — Mañana'],['aidedPartial',1,'Movilizaciones con ayuda — levantamiento parcial — Tarde'],['aidedPartial',2,'Movilizaciones con ayuda — levantamiento parcial — Noche']
 ];
 ...
 function decorate(){
  const table=document.querySelector('.mob-table');if(!table)return;
  const state=getState(),screen=Math.max(0,Math.min(11,state.uiScreen));state.uiScreen=screen;const [key,shift,title]=screens[screen];
  ...
  [...table.querySelectorAll('tbody tr')].forEach(row=>[...row.children].forEach((cell,i)=>{cell.style.display=(i===0||i===screen+1||row.classList.contains('mob-section'))?'':'none';}));
