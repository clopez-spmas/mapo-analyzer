...
function render(){const host=$('mapoSimulation');let current,original;try{original=calc(baseData,true);current=simulationChanged?calc(simulationData,false):original;}catch(err){host.innerHTML=`<div class="section-heading"><h2>Simulación de mejoras del índice MAPO</h2><button type="button" id="closeMapoSimulation" class="secondary">Volver a resultados</button></div><div class="error">${esc(err.message)}</div>`;host.hidden=false;showScreen();$('closeMapoSimulation').onclick=restoreResults;return;}const mr=window.MAPOMultiRoom,s=mr?.state,room=s?.rooms?.[s.active],roomLabel=s?.rooms?.length>1?' — '+(room?.name||('Sala '+(s.active+1))):'';let h=`<div class="section-heading"><div><h2>Simulación de mejoras del índice MAPO${roomLabel}</h2><p>Modifique las condiciones concretas. El estudio original no se modifica.</p></div><button type="button" id="closeMapoSimulation" class="secondary">Volver a resultados</button></div><div class="simulation-summary"><div><span>MAPO actual</span><strong>${original.mapo.toFixed(2)}</strong></div><div>→</div><div><span>MAPO simulado</span><strong class="sim-score">${current.mapo.toFixed(2)}</strong></div></div>${summaryChanges()}`;h+=['fs','fa','fc','famb','ff'].map(k=>factor(k,current.f,simulationData)).join('');h+='<div class="actions"><button type="button" id="recalcMapoSimulation">Recalcular simulación</button><button type="button" id="resetMapoSimulation" class="secondary">Restablecer valores originales</button></div>';host.innerHTML=h;showScreen();
