/* MAPO Analyzer — controlador único de navegación.
   RESPONSABILIDADES:
   - Estado inicial y transiciones entre pantallas.
   - Anterior/Siguiente en todas las pantallas del estudio.
   - Transición identificación del estudio -> accesos directos -> módulos.
...
(function(){'use strict';
 ...
 function total(){const s=typeof selectedStudy!=='undefined'?selectedStudy:null;return s&&window.MAPO_STUDIES?.[s]?.steps?.length||10;}
 function setInitialScreen(){
   const ids=['accessScreen','studySelection','roomSetup','moduleHub','studyPanel','result','globalResults','mapoSimulation','templateAdmin'];
   ids.forEach(id=>{const e=$(id);if(e)e.hidden=true;});
 ...
 function init(){setInitialScreen();bindStandard();studySelection();bindRoomSetup();roomSetup();const panel=$('studyPanel');if(panel){new MutationObserver(()=>{if(!$('studyPanel').hidden)bindStandard();}).observe(panel,{subtree:true,childList:true});}setInterval(()=>{if(!$('studyPanel')?.hidden)bindStandard();},250);}
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
 window.MAPONavigation={previous,next,bindStandard,bindRoomSetup,setInitialScreen};
})();