...
(function(){
  ...
  function filterSelectedMobilization(){
    ...
    const kindByCategory=['ta','pa','tm','pm'],selectedKind=kindByCategory[sel.category];
    document.querySelectorAll('#formContainer .task-block').forEach(block=>{
      block.querySelectorAll('.task-shift').forEach(shift=>{
    ...
    const turns=['Mañana','Tarde','Noche'],labels=['Movilizaciones totales con ayuda','Movilizaciones parciales con ayuda','Movilizaciones totales sin ayuda','Movilizaciones parciales sin ayuda'];
  ...
  window.renderStep=function(){
    ...
    if(step?.custom==='mobilizations'){
      ...
      renderMobilizations();filterSelectedMobilization();
