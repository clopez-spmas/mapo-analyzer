...
function getResult(){const s=getState();return s&&s.result&&Object.keys(s.result).length?s.result:null;}
const labels={
...
['OP',labels.OP,n(op).toFixed(3)],['ST',labels.ST,n(t.st)],['LTA',labels.LTA,n(t.lta)],['SP',labels.SP,n(t.sp)],['LPA',labels.LPA,n(t.lpa)],['% LTA','Porcentaje de movilizaciones de pacientes no colaboradores respecto al total',pct(t.pLTA)],['% LPA','Porcentaje de movilizaciones parciales realizadas con ayuda respecto al total',pct(t.pLPA)],['FS',labels.FS,n(result.fs).toFixed(2)],['FA',labels.FA,n(result.fa).toFixed(2)],['FC',labels.FC,n(result.fc).toFixed(2)],['Famb',labels.Famb,n(result.famb).toFixed(2)],['FF',labels.FF,n(result.ff).toFixed(2)]];return '<table class="mapo-report-table mapo-result-table"><thead><tr><th>Indicador</th><th>Descripción</th><th>Valor</th></tr></thead><tbody>'+rows.map(r=>'<tr><th>'+r[0]+'</th><td>'+r[1]+'</td><td>'+r[2]+'</td></tr>').join('')+'</tbody></table>';}
...
function render(){const r=$('result'),result=getResult();if(!r||!result)return;const breakdown=$('breakdown');if(!breakdown)return;const state=getState(),form=state?.form||{};const mr=window.MAPOMultiRoom,s=mr?.state,room=s?.rooms?.[s.active],multi=Array.isArray(s?.rooms)&&s.rooms.length>1;const title=$('result')?.querySelector('.section-heading h2');if(title)title.textContent=multi?'Resultado — '+(room?.name||('Sala '+(s.active+1))):'Resultado de la evaluación';$('mapoValue').textContent=`MAPO = ${n(result.mapo).toFixed(2)}`;$('classification').textContent=`Nivel de exposición: ${result.nivel||''}`;breakdown.innerHTML=resultHtml(result,form.op)+'<div class="report-table-actions" style="margin-top:10px"><button type="button" id="copyMapoResultTable">Copiar tabla para Word</button></div>';const b=$('copyMapoResultTable');if(b)b.onclick=()=>copyResult(b);}
