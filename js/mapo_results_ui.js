...
function render(){const r=$('result'),result=getResult();if(!r||!result)return;const breakdown=$('breakdown');if(!breakdown)return;const state=getState(),form=state?.form||{};$('mapoValue').textContent=`MAPO = ${n(result.mapo).toFixed(2)}`;$('classification').textContent=`Nivel de exposición: ${result.nivel||''}`;breakdown.innerHTML=resultHtml(result,form.op)+'<div class="report-table-actions" style="margin-top:10px"><button type="button" id="copyMapoResultTable">Copiar tabla para Word</button></div>';const b=$('copyMapoResultTable');if(b)b.onclick=()=>copyResult(b);}
