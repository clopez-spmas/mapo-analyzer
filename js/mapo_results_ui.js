...
function render(){const r=$('result'),result=getResult();if(!r||r.hidden||!result)return;const breakdown=$('breakdown');if(!breakdown)return;const form=typeof window.MAPOReportState==='function'?window.MAPOReportState()?.form:(window.formData||{});breakdown.innerHTML=resultHtml(result,form?.op)+'<div class="report-table-actions" style="margin-top:10px"><button type="button" id="copyMapoResultTable">Copiar tabla para Word</button></div>';const b=$('copyMapoResultTable');if(b)b.onclick=()=>copyResult(b);}
